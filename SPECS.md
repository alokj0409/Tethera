...
All positive safe-integer levels use these implemented parameters:
...
- Target radius: `14`; hazard radius: `12`; wormhole radius: `18`; pulsar radius: `20`.
...
Levels 13-20 add `min(1 + floor((level - 13) / 3), 3)` bouncers. Their centers consume additional points from the same 70-unit Poisson-disc set. Each segment has seeded orientation and length in `[64, 92)` logical units.
...
Levels 51-65 add one gravity pole, 66-80 add two, and 81+ add three. Each pole has radius `20`, seeded strength in `[160000, 200000)`, seeded period in `[2.2s, 2.8s)`, seeded phase, active duration `0.55s`, softening `36`, and maximum acceleration `260 logical units/s^2`. During the active interval, envelope intensity is `sin(pi * pulseAge / 0.55)`. For distance `d`, acceleration magnitude is `min(260, strength / (d^2 + 36^2)) * intensity` toward the pole. Multiple pole acceleration vectors add. Free-flight acceleration changes both velocity components; a held tether receives only `dot(acceleration, unitTangent) * dt`, retaining its rigid radius while warping angular speed.

The PRD tier table promises `6+` tethers for Linear Orbits, while its sample formula yields 4 or 5 for those levels. The implemented minimum of 6 follows the tier table. Victory now advances without a milestone loop for every positive safe-integer level.
